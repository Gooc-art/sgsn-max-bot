...
import max_bot
from sud_export import Row, is_sgsn_party


CHAT_ID_FILE = Path(os.environ.get("SGSN_WEEKLY_CHAT_ID_FILE", "~/.config/sgsn-max-bot/weekly-chat-id")).expanduser()
...
def sgsn_rows(csv_path: Path) -> list[list[str]]:
...
def run_export(start: date, end: date, outdir: Path, timeout: int) -> None:
    ...
    result = subprocess.run(cmd, text=True, capture_output=True, timeout=int(os.environ.get("SGSN_EXPORT_TIMEOUT_SECONDS", os.environ.get("SUD_EXPORT_TIMEOUT_SECONDS", str(4 * 60 * 60)))))
...
def notify(chat_id: str, start: date, end: date, outdir: Path, count: int, force: bool = False) -> None:
    ...
    if count:
        ...
        text = f"Тестовый прогон СГСН на {start:%d.%m.%Y}-{end:%d.%m.%Y}: СГСН не найдена. Отправляю Excel для проверки доставки."
    max_bot.send_text(target, text)
    max_bot.upload_and_send_file(target, outdir / "report.xlsx", "Excel-отчет")


def weekly_chat_id() -> str:
    env_chat_id = os.environ.get("SGSN_WEEKLY_CHAT_ID", os.environ.get("SUD_WEEKLY_CHAT_ID", "")).strip()
    if env_chat_id:
        return env_chat_id
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--timeout", type=int, default=int(os.environ.get("SGSN_HTTP_TIMEOUT_SECONDS", os.environ.get("SUD_HTTP_TIMEOUT_SECONDS", "20"))))
    ...
    outdir = Path(args.outdir or f"output/weekly-sgsn/{start.isoformat()}")

    if not args.skip_export:
        run_export(start, end, outdir, args.timeout)
    found = sgsn_rows(outdir / "report.csv")
    print(f"sgsn_rows={len(found)} xlsx={outdir / 'report.xlsx'}")

    chat_id = weekly_chat_id()
    if (found or args.force_send) and not args.no_send:
        if not re.fullmatch(r"-?\d+", chat_id):
            raise RuntimeError("SGSN_WEEKLY_CHAT_ID is not set")
